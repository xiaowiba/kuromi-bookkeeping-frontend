<template>
  <a-modal v-model:visible="visible" title="关联报销" fullscreen :footer="false" unmount-on-close>
    <div class="reimburse-modal-content">
      <!-- 左侧：搜索 + 信息卡 + 候选列表 -->
...
const selectedPublicUserId = ref('')
...
// 报销关联默认只查询“与当前垫付金额完全一致”的候选支出，
// 这样用户切换到目标账户后，左侧列表会直接缩小到最可能关联的那几条记录。
...
async function loadCandidates() {
  ...
  try {
    ...
    const res = await listReimburseCandidates(params)
...
async function onConfirmLink() {
  if (!selectedDetailId.value) {
    Message.warning('请先选择要关联的报销明细')
    return
  }
  try {
    await linkReimburse({
      advanceDetailId: String(props.sourceDetail.id),
      reimburseDetailId: selectedDetailId.value,
    })
